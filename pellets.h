...
public:
    Pellet(int x, int y, float radius, sf::Color c) : position(x, y), eaten(false), ghostEaten(false)
    {
    ...
    bool getGhostEaten() { return ghostEaten; }
    void setGhostEaten() { ghostEaten = true; }
...
public:
    Pellets()
    {
        ...
        player_pellet.emplace_back(2 * 30, 20 * 30, 10, sf::Color::Cyan);
        player_pellet.emplace_back(29 * 30, 6 * 30, 10, sf::Color::Cyan);
        player_pellet.emplace_back(2 * 30, 9 * 30, 10, sf::Color::Cyan);
    ...
    {
        if (!player_pellet[pIdx].getEaten())
            window.draw(player_pellet[pIdx].pellet);
        if (!ghost_pellet[gIdx].getEaten())
            window.draw(ghost_pellet[gIdx].pellet);
    }

    void setCollision_player(int index) { player_pellet[index].setEaten(true); }
    ...
    void setPosition_player(int index)
    {
        player_pellet[index].position.setX(-1);
        player_pellet[index].position.setY(-1);
    }

    void setCollision_ghost(int index, bool status)
    {
        ghost_pellet[index].setEaten(status);
        ghost_pellet[index].setGhostEaten();
    }
    ...
    void setPosition_ghost(int index)
    {
        ghost_pellet[index].position.setX(-1);
        ghost_pellet[index].position.setY(-1);
    }
};
