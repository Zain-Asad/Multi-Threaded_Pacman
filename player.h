...
public:
    Pacman(const int width, const int height, pthread_mutex_t pacman, pthread_mutex_t coin, pthread_mutex_t p) : 
    ...
    void drawPacman(sf::RenderWindow &window) {
        pacman.setRadius(pacmanRadius); // Increased size
    ...
    void move(GameBoard& game, Pellets& pellet, int index, sf::Clock& c) {
        ...
        switch (direction) {
            case UP:
                if(collidesWithPellet(pellet, index, pacmanPosX, pacmanPosY-pacmanSpeed)) { pellet.setCollision_player(index); pellet.setPosition_player(index); c.restart();  }
                if(collideWithCoin(game, pacmanPosX, pacmanPosY-pacmanSpeed)) { update_score(); }
                if(!collideWithWall(game, pacmanPosX, pacmanPosY-pacmanSpeed-3)) {   pacmanPosY -= pacmanSpeed;   }
                break;
            case DOWN:
                if(collidesWithPellet(pellet, index, pacmanPosX, pacmanPosY+pacmanSpeed)) { pellet.setCollision_player(index); pellet.setPosition_player(index); c.restart(); }
                if(collideWithCoin(game, pacmanPosX, pacmanPosY+pacmanSpeed)) { update_score(); }
                if(!collideWithWall(game, pacmanPosX, pacmanPosY+pacmanSpeed+20)) { pacmanPosY += pacmanSpeed; }
                break;
            case LEFT:
                if(collidesWithPellet(pellet, index, pacmanPosX-pacmanSpeed, pacmanPosY)) { pellet.setCollision_player(index); pellet.setPosition_player(index);  c.restart();}
                if(collideWithCoin(game, pacmanPosX-pacmanSpeed, pacmanPosY)) { update_score(); }
                if(!collideWithWall(game, pacmanPosX-pacmanSpeed-3, pacmanPosY)) {  pacmanPosX -= pacmanSpeed;   }
                break;
            case RIGHT:
                if(collidesWithPellet(pellet, index, pacmanPosX+pacmanSpeed, pacmanPosY)) { pellet.setCollision_player(index); pellet.setPosition_player(index);  c.restart();}
                if(collideWithCoin(game, pacmanPosX+pacmanSpeed, pacmanPosY)) { update_score(); }
                if(!collideWithWall(game, pacmanPosX+pacmanSpeed+20, pacmanPosY)) { pacmanPosX += pacmanSpeed; }
                break;
        }
        pthread_mutex_unlock(&pacman_input);
    }
